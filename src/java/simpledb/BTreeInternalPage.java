...
public class BTreeInternalPage extends BTreePage {
    ...
    public byte[] getPageData() {
        ...
        // padding
        int zerolen =
                BufferPool.getPageSize() - (INDEX_SIZE + 1 + header.length + td.getFieldType(keyField).getLen() * (
                        keys.length - 1) + INDEX_SIZE * children.length);
    ...
    public void insertEntry(BTreeEntry e) throws DbException {
                ...
                if (children[i] == e.getLeftChild().getPageNumber() || children[i] == e.getRightChild()
                    ...
                    if (i > 0 && keys[i].compare(Op.GREATER_THAN, e.getKey())) {
                        throw new DbException(
                                "attempt to insert invalid entry with left child " + e.getLeftChild().getPageNumber()
                                        + ", right child " + e.getRightChild().getPageNumber() + " and key " + e
                                        .getKey()
        ...
        if (lessOrEqKey == -1) {
            throw new DbException("attempt to insert invalid entry with left child " + e.getLeftChild().getPageNumber()
                    + ", right child " + e.getRightChild().getPageNumber() + " and key " + e.getKey()
                    + " HINT: one of these children must match an existing child on the page"
