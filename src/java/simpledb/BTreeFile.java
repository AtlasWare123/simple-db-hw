...
public class BTreeFile implements DbFile {
    ...
    private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid,
        ...
        while (iter.hasNext()) {
            ...
            if (cur.getKey().compare(Op.GREATER_THAN_OR_EQ, f)) {
                break;
            } else {
                entry = cur;
            }
    ...
    protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page,
        ...
        // move second half of tuples to new leaf page
        BTreeLeafPageIterator iter = (BTreeLeafPageIterator) page.iterator();
        for (int i=0; i<page.getNumTuples() / 2 && iter.hasNext(); i++) {
            iter.next();
        }
        while (iter.hasNext()) {
            Tuple tuple = iter.next();
            page.deleteTuple(tuple);  // delete must be first because insert op will update record id
            siblingLeafPage.insertTuple(tuple);
        }

        // create and set newly created sibling leaf page's parent
    ...
    protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling, BTreeInternalPage parent,
        ...
        int total = leftPage.getNumTuples() + rightPage.getNumTuples();
        BTreeLeafPageIterator leftIter = (BTreeLeafPageIterator) leftPage.iterator();
        int leftIdx = 0;
        while (leftIter.hasNext() && leftIdx < total / 2) {
            leftIdx++;
            leftIter.next();
        }

        // steal elements
        if (leftIdx < total / 2) {  // steal some elements from right page to left page
            BTreeLeafPageIterator rightIter = (BTreeLeafPageIterator) rightPage.iterator();
            for (int i=leftIdx; i<total / 2 && rightIter.hasNext(); i++) {
    ...
    protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        // insert parentEntry to right page
    ...
    protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage leftPage,
        ...
        BTreeLeafPageIterator iter = (BTreeLeafPageIterator) rightPage.iterator();
        while (iter.hasNext()) {
            Tuple tuple = iter.next();
            rightPage.deleteTuple(tuple);
            leftPage.insertTuple(tuple);
        ...
        // update relationship between pages
        BTreePageId rightSiblingId = rightPage.getRightSiblingId();
        leftPage.setRightSiblingId(rightSiblingId);
        if (rightSiblingId != null) {
            BTreeLeafPage rightSiblingPage = (BTreeLeafPage) this.getPage(tid, dirtypages, rightSiblingId, Permissions.READ_WRITE);
