...
public class OperatorCardinality {
    ...
    private static boolean updateFilterCardinality(Filter f, Map<String, Integer> tableAliasToId,
        ...
        if (tableId != null) {
            selectivity = tableStats.get(Database.getCatalog().getTableName(tableId))
                    .estimateSelectivity(Database.getCatalog().getTupleDesc(tableId).fieldNameToIndex(pureFieldName),
    ...
    private static boolean updateJoinCardinality(Join j, Map<String, Integer> tableAliasToId,
        ...
        boolean child1HasJoinPK =
                Database.getCatalog().getPrimaryKey(tableAliasToId.get(tableAlias1)).equals(pureFieldName1);
        boolean child2HasJoinPK =
                Database.getCatalog().getPrimaryKey(tableAliasToId.get(tableAlias2)).equals(pureFieldName2);
        ...
        j.setEstimatedCardinality(JoinOptimizer
                .estimateTableJoinCardinality(j.getJoinPredicate().getOperator(), tableAlias1, tableAlias2,
                        pureFieldName1, pureFieldName2, child1Card, child2Card, child1HasJoinPK, child2HasJoinPK,
    ...
    private static boolean updateAggregateCardinality(Aggregate a, Map<String, Integer> tableAliasToId,
        ...
        if (tableId != null) {
            groupFieldAvgSelectivity = tableStats.get(Database.getCatalog().getTableName(tableId))
                    .avgSelectivity(Database.getCatalog().getTupleDesc(tableId).fieldNameToIndex(pureFieldName),
                            Predicate.Op.EQUALS);
